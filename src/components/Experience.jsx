...
const Experience = () => {
  return (
    <div className="border-b border-neutral-900 pb-4">
        <motion.h2 whileInView={{ opacity: 1, y:0 }} initial={{opacity:0, y:-100}} transition={{duration: 0.5}} className="my-20 text-center text-4xl text-white-400">Experiences</motion.h2>
        <div>
                ...
                <div key={index} className="mb-8 flex flex-wrap lg:justify-center">
                    ...
                    <motion.div whileInView={{ opacity: 1, x:0 }} initial={{opacity:0, x:100}} transition={{duration: 0.5}} className='w-full max-w-xl lg:w-3/4'>
                        ...
                        {experience.technologies.map((tech, index) =>(
                            <span key={index} className='mr-2 mt-4 rounded bg-neutral-800 px-2 py-1 text-sm font-medium text-purple-800'>{tech}</span>
                        ))}
                    </motion.div>
                </div>
            ))}
